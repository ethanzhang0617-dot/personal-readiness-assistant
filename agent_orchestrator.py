...
_ANSWER_ADDENDUM = (
    ...
    "below, together with the structured product context. Use only those results and that context for any personal "
    "number, unit or period, and never invent a value that is not there. The recommendation, the session demand, the "
    "RIR guidance, the training load and any safety classification were already decided by the deterministic system: "
    "explain them, never replace them. If a fact the user asked for is not available, say it is not available. Answer "
    "the question directly in two to five sentences."
)
